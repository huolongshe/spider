...
class TileManager:
    def __init__(self, g):
        self.g = g
        self._tile_cache = {}
        self._cache_path = os.path.join(self.g.data_path, 'mapcache')
    ...
    def get_tile_sync(self, tile_url, tile_id, file_format):
        ...
        if tile_bmp and tile_bmp is not self.g.tile_transparent_bmp:
            return tile_bmp

        if tile_bmp is self.g.tile_transparent_bmp:  # 之前可能是因为超时等原因缓存的透明图标，重新尝试下载
