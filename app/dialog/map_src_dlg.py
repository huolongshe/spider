...
class MapSrcDlg(wx.Dialog):
    ...
    def on_trans_checked(self, event):
        ...
        if self.trans_src_box.IsChecked(sel_index):
            g.map_list_trans[sel_index].is_visible = True
        else:
            g.map_list_trans[sel_index].is_visible = False
        self.repaint_map()
    ...
    def on_click_trans_up(self, event):
        ...
        if sel_index > 0:
            ...
            self.trans_src_box.Insert(g.map_list_trans[sel_index].name, (sel_index - 1))
            self.trans_src_box.SetSelection(sel_index - 1)
            if is_checked:
                self.trans_src_box.Check(sel_index - 1)
            g.map_list_trans.insert(sel_index - 1, g.map_list_trans.pop(sel_index))
            self.repaint_map()
    ...
    def on_main_selected(self, event):
        ...
        for i in range(len(g.map_list_main)):
            if i == sel_index:
                g.map_list_main[i].is_visible = True
            else:
                g.map_list_main[i].is_visible = False
        self.repaint_map()

    def repaint_map(self):
        g.map_canvas.repaint(mode=const.REDRAW_MAP)
        g.frame.show_status_info(show_map=True)
