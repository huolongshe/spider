...
class SrtmWptDlg(wx.Dialog):
    def __init__(self, g, wpt):
        ...
        self._ctl_gauge_point = wx.Gauge(self, -1, self.g.srtm_mgr.PROGRESS_RANGE, (30, 50), (540, 30))
    ...
    def download_wpt_alt_main_thread(self):
        ...
        if self.wpt.alt <= 0:  # 本应用中假定海拔高度都应大于0
            ...
            thread = threading.Thread(target=self.g.srtm_mgr.get_alt_network, args=(self.wpt.lon, self.wpt.lat))
            thread.setDaemon(True)
            thread.start()
        ...
        self.g.frame.repaint(const.REDRAW_COPY)
        self.Destroy()